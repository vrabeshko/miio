...
module.exports.Ambilight = Thing.mixin(Parent => class extends Parent
	.with(MiioApi, Colorable)
{

	constructor(...args) {
		super(...args);

		this.defineProperty('bg_power', {
			name: 'bgPower',
			mapper: v => v === 'on'
		});
		...
		this.defineProperty('bg_ct', {
			name: 'bgColorTemperature',
			mapper: parseInt
		});

		this.defineProperty('bg_lmode', {
			name: 'bgColorMode',
			mapper: v => {
				v = parseInt(v);
				switch(v) {
		...
		this.defineProperty('bg_bright', {
			name: 'bgBrightness',
			mapper: parseInt
		});

		this.defineProperty('bg_rgb', {
			name: 'bgColorRGB',
			mapper: rgb => {
				rgb = parseInt(rgb);
		...
		this.defineProperty('bg_hue', {
			name: 'bgColorHue',
			mapper: parseInt
		});

		this.defineProperty('bg_sat', {
			name: 'bgColorSaturation',
			mapper: parseInt
		});
	}
	...
	changeBgPower(power) {
		...
		).then(res => MiioApi.checkOk(res));
	}

	changeBgColor(color, options) {
		// if(color.is('temperature')) {
		// 	/*
		// 	 * User has request a color via temperature
		// 	 */
		// 	const range = this.colorTemperatureRange;
		// 	const temperature = Math.min(Math.max(color.temperature.kelvins, range.min), range.max);

		// 	return this.call('bg_set_ct_abx', Yeelight.withEffect(temperature, options.duration), {
		// 		refresh: [ 'bgColorTemperature', 'bgColorMode' ]
		// 	}).then(MiioApi.checkOk);
		// } else if(color.is('hsl')) {
		// 	/*
		// 	 * User has requested hue and saturation
		// 	 */
		// 	return this.call('bg_set_hsv', Yeelight.withEffect([ color.hue, color.saturation ], options.duration), {
		// 		refresh: [ 'bgColorHue', 'bgColorSaturation', 'bgColorMode' ]
		// 	}).then(MiioApi.checkOk);
		// } else {
			/*
			 * Fallback to applying via RGB.
			...
			const rgb = color.red * 65536 + color.green * 256 + color.blue;
			return this.call('bg_set_rgb', Yeelight.withEffect(rgb, options.duration), {
				refresh: [ 'bgColorRGB', 'bgColorMode' ]
			}).then(MiioApi.checkOk);
		// }
	}
});
